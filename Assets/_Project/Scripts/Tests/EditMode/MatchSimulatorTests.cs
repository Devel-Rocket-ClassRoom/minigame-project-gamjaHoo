...
namespace FMLite.Tests
{
    public class MatchSimulatorTests
    {
        ...
        public void T1_Determinism_SameSeedSameResult()
        {
            ...
            CollectionAssert.AreEqual(r1.homeStarting11, r2.homeStarting11, "T1: homeStarting11 결정적");
            CollectionAssert.AreEqual(r1.awayStarting11, r2.awayStarting11, "T1: awayStarting11 결정적");
            Assert.AreEqual(r1.playerStats.Count, r2.playerStats.Count, "T1: playerStats count 결정적");
            for (int i = 0; i < r1.playerStats.Count; i++)
            {
                Assert.AreEqual(r1.playerStats[i].playerId, r2.playerStats[i].playerId, $"T1: playerStats[{i}].playerId");
                Assert.AreEqual(r1.playerStats[i].goals,    r2.playerStats[i].goals,    $"T1: playerStats[{i}].goals");
            }
        ...
        public void T2_StartingEleven_TopByCAExcludingInjured()
        {
            ...
            var benchPlayers = homeClub.seniorSquadIds
                .Where(id => !result.homeStarting11.Contains(id))
                .Select(id => state.GetPlayer(id))
                .ToList();
            int starting11MinCA = result.homeStarting11
                .Select(id => state.GetPlayer(id).currentAbility).Min();
        ...
        public void T2b_StartingEleven_ExcludesInjuredPlayers()
        {
            ...
            var top5 = home.seniorSquadIds
                .Select(id => state.GetPlayer(id))
                .OrderByDescending(p => p.currentAbility)
                .Take(5).ToList();
            foreach (var p in top5) p.state.injury.injuryTypeId = 1;   // 부상 상태
        ...
        public void T2c_StartingEleven_FewerThan11WhenSquadShort()
        {
            ...
            var state = new GameState { randomSeed = 1, currentDate = new System.DateTime(2025, 8, 15) };
            var home = NewClub(1, name: "Home", squadSize: 5, ca: 100);
            var away = NewClub(2, name: "Away", squadSize: 25, ca: 100);
            state.AddClub(home); state.AddClub(away);
            int nextId = 1;
            ...
            var match = new Match { id = 1, homeClubId = 1, awayClubId = 2, type = CompetitionType.League };
        ...
        public void T3_StrongTeamWinRate_OverThreshold()
        {
            ...
            int strongWinsAsHome = SimulateManyMatches(
                strongCA: 170, weakCA: 90, strongIsHome: true, matchCount: 200, globalSeed: 100);
            Assert.GreaterOrEqual(strongWinsAsHome, 130,
            ...
            int strongWinsAsAway = SimulateManyMatches(
                strongCA: 170, weakCA: 90, strongIsHome: false, matchCount: 200, globalSeed: 200);
            Assert.GreaterOrEqual(strongWinsAsAway, 100,
        ...
        public void T4_EqualTeams_HomeAdvantageShowsInWinRates()
        {
            ...
            for (int i = 0; i < 1000; i++)
            {
                ...
                var (state, match) = BuildState(homeCA: 100, awayCA: 100, seed: matchSeed, matchId: i);
            ...
            Assert.Greater(homeWins, awayWins,
        ...
        public void T5_GoalDistribution_MatchesExpectedRanges()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                totalGoals += goals;
                if (goals == 0) scorelessMatches++;
                if (goals >= 5) highScoringMatches++;
            }
            ...
            Assert.That(avgGoals, Is.EqualTo(3.0).Within(0.2),
                $"T5: 평균 골수 ≈ 3.0 ±0.2 (실측 {avgGoals:F2})");
            Assert.That(scorelessRate, Is.InRange(0.02, 0.10),
                $"T5: 무득점 비율 2~10% (이론 5%, 실측 {scorelessRate:P1})");
            Assert.That(highScoringRate, Is.InRange(0.08, 0.25),
        ...
        public void T6_ScorerDistribution_AT_GreaterThan_MF_GreaterThan_DF_AndNoGK()
        {
            ...
            var lineCounts = new Dictionary<Line, int> { [Line.GK] = 0, [Line.DF] = 0, [Line.MF] = 0, [Line.AT] = 0 };
            const int N = 500;
            for (int i = 0; i < N; i++)
            {
                int matchSeed = seedGen.Next();
            ...
            Assert.Greater(atRate, mfRate, $"T6: AT 비율 > MF 비율 (at={atRate:P1} mf={mfRate:P1})");
            Assert.Greater(mfRate, dfRate, $"T6: MF 비율 > DF 비율 (mf={mfRate:P1} df={dfRate:P1})");
            Assert.AreEqual(0, lineCounts[Line.GK], $"T6: GK 득점 = 0 (가중치 0). gkRate={gkRate:P1}");
            // 라인별 가중치 합 (인원 × weight × avg CA/100):
            //   AT: 6 × 5.0 × ~1.07 ≈ 32 / MF: 8 × 1.5 × ~1.07 ≈ 13 / DF: 8 × 0.4 × ~1.07 ≈ 3.4
            //   총합 ≈ 48 → AT 비율 ≈ 67%, MF ≈ 27%, DF ≈ 7%
            Assert.That(atRate, Is.InRange(0.55, 0.80),
        ...
        public void T7_PlayerMatchStat_FilledCorrectlyForV01()
        {
            ...
            Assert.AreEqual(r.homeStarting11.Count + r.awayStarting11.Count, r.playerStats.Count,
                "T7: starting11 합 == playerStats.Count");

            int goalsInStats = r.playerStats.Sum(ps => ps.goals);
            Assert.AreEqual(r.homeScore + r.awayScore, goalsInStats,
        ...
        private (GameState, Match) BuildState(int homeCA, int awayCA, int seed, int matchId)
        {
            ...
            var match = new Match { id = matchId, homeClubId = 1, awayClubId = 2, type = CompetitionType.League };
            return (state, match);
        }
        ...
        private static int AddDiverseSquad(GameState state, Club club, int nextId)
        {
            ...
            {
                (Position.GK, 3),
                (Position.CB, 4), (Position.LB, 2), (Position.RB, 2),
                (Position.DM, 2), (Position.CM, 3), (Position.LM, 1), (Position.RM, 2),
                (Position.LW, 1), (Position.RW, 1), (Position.ST, 3), (Position.CF, 1),
            };
        ...
        private static Club NewClub(int id, string name, int squadSize = 25, int ca = 100) =>
            new Club { id = id, name = name, reputation = 60 };

        private static int AddPlayersToClub(GameState state, Club club, int ca, int nextId, int count)
        {
        ...
        private static Player NewPlayer(int id, Position pos, int ca) => new Player
        {
            id = id,
            currentAbility = ca,
            potentialAbility = ca + 10,
            info = new PersonalInfo { primaryPosition = pos, firstName = "F", lastName = "L" },
        ...
        private int SimulateManyMatches(int strongCA, int weakCA, bool strongIsHome, int matchCount, int globalSeed)
        {
            ...
            for (int i = 0; i < matchCount; i++)
            {
                ...
                var r = MatchSimulator.Simulate(match, state, _balance);
                bool strongWon = strongIsHome ? (r.homeScore > r.awayScore) : (r.awayScore > r.homeScore);

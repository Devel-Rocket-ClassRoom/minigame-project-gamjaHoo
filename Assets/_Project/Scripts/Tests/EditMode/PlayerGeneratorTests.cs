...
namespace FMLite.Tests
{
    public class PlayerGeneratorTests
    {
        ...
        public void T1_BigClubVeteranST_CAAndStatsInRange()
        {
            ...
            Assert.That(player.currentAbility, Is.InRange(120, 155), "T1: CA in [120, 155]");
            Assert.That(
        ...
        public void T2_SmallClubYouthCM_CAAndNationalityInRange()
        {
            ...
            Assert.That(player.currentAbility, Is.InRange(50, 90), "T2: CA in [50, 90]");
            // PA 절댓값은 seed·noise 의존. 핵심 검증은 PA >= CA (알고리즘 보장) + 합리적 상한.
            Assert.That(player.potentialAbility, Is.InRange(50, 150), "T2: PA in [50, 150]");

            var espPool = GameDatabase.GetNamePool(4); // ESP countryId=4
            Assert.That(
                espPool.firstNames,
                Contains.Item(player.info.firstName),
        ...
        public void T4_Determinism_DifferentSeedDifferentResult()
        {
            ...
            bool anyDiff =
                p1.currentAbility != p2.currentAbility
        ...
        public void T5_Distribution_1000OutfieldPlayers()
        {
            ...
            Assert.That(avgGap, Is.InRange(14.0, 32.0), $"T5: PA-CA 갭 평균 (actual={avgGap:F1})");

            // 트레잇 보유 비율 30% ±5% (1000샘플 σ≈1.5%, ±5%는 약 3σ 허용)
            double traitRatio = players.Count(p => p.traitIds.Count > 0) / (double)n;
            Assert.That(
                traitRatio,
                Is.InRange(0.25, 0.35),
                $"T5: 트레잇 보유 비율 30% ±5% (actual={traitRatio:P1})"
            );

            // 늦깎이형(id=1) + 조숙형(id=2) 동시 보유 = 0건
            int conflictCount = players.Count(p =>
                p.traitIds.Contains(1) && p.traitIds.Contains(2)
            );
            ...
            Assert.That(r, Is.GreaterThan(0.6), $"T5: CA-Stats 상관계수 > 0.6 (actual={r:F3})");
        }
        ...
        public void T7_GkSecondaryPositions_AlwaysZero()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.AreEqual(
                    0,
                    p.info.secondaryPositions.Count,

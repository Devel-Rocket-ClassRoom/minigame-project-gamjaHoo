    ...
    public class ClubGeneratorTests
    {
        ...
        public void T1_Determinism_SameSeedSameResult()
        {
            ...
            for (int i = 0; i < r1.Clubs.Count; i++)
            {
                ...
                Assert.AreEqual(r1.Clubs[i].reputation,     r2.Clubs[i].reputation,     $"T1: club[{i}].reputation");
                Assert.AreEqual(r1.Clubs[i].finance.money,  r2.Clubs[i].finance.money,  $"T1: club[{i}].finance.money");
                Assert.AreEqual(r1.Clubs[i].facilities.scoutLevel, r2.Clubs[i].facilities.scoutLevel,
            ...
            for (int i = 0; i < r1.Players.Count; i++)
            {
                ...
                Assert.AreEqual(r1.Players[i].currentAbility,    r2.Players[i].currentAbility,    $"T1: player[{i}].CA");
                Assert.AreEqual(r1.Players[i].info.firstName,    r2.Players[i].info.firstName,    $"T1: player[{i}].firstName");
                Assert.AreEqual(r1.Players[i].info.primaryPosition, r2.Players[i].info.primaryPosition,
        ...
        public void T2b_VariableClubCount_TierSumMatchesN()
        {
            foreach (int n in new[] { 10, 12, 24 })
            {
                ...
                var result = ClubGenerator.Generate(new Random(42), cfg, _balance, _testDate, 1, 1, 1);
                Assert.AreEqual(n, result.Clubs.Count, $"T2b: clubCount={n} 일 때 정확히 {n}개 생성");
                Assert.AreEqual(n * 25, result.Players.Count, $"T2b: clubCount={n} 일 때 선수 {n * 25}명");
            }
        }
        ...
        public void T3_SquadComposition_AllClubsMatchFormationConstraints()
        {
            ...
            foreach (var club in result.Clubs)
            {
                ...
                var posCounts = club.seniorSquadIds
        ...
        {
            var result = Generate(new Random(42));
            var ages   = result.Players
                .Select(p => _testDate.Year - p.info.birthDate.Year)
                .ToList();
            ...
            Assert.That(youthR, Is.InRange(0.15, 0.25),
                        $"T4: youth 비율 20% ±5% (actual={youthR:P1})");
            Assert.That(primeR, Is.InRange(0.55, 0.65),
                        $"T4: prime 비율 60% ±5% (actual={primeR:P1})");
            Assert.That(vetR,   Is.InRange(0.15, 0.25),
        ...
        public void T5_NationalityDistribution_500PlayersBatch()
        {
            ...
            Assert.That(ratio, Is.InRange(0.64, 0.76),
                        $"T5: ENG 비율 70% ±6% (actual={ratio:P1})");
        }
        ...
        public void T6_ReputationCorrelations_ClubCount20()
        {
            ...
            Assert.That(rMoney, Is.InRange(0.5, 0.99),
                        $"T6: corr(rep, money) in [0.5, 0.99] (actual={rMoney:F3})");
            Assert.That(rScout, Is.InRange(0.4, 0.99),
        ...
        public void T7_IdUniquenessAndMonotonic()
        {
            ...
            Assert.AreEqual(clubIds.Count,   clubIds.Distinct().Count(),
                            "T7: club id 중복 없음");
            Assert.AreEqual(playerIds.Count, playerIds.Distinct().Count(),
                            "T7: player id 중복 없음");
            ...
            foreach (var club in result.Clubs)
            {
                foreach (var id in club.seniorSquadIds)
                    Assert.IsTrue(playerIdSet.Contains(id),
        ...
        private static void RegisterCountriesAndNamePools()
        {
            ...
            var defs = new[]
            {
                ...
                (3, "GER",
                 new[]{ "Hans","Michael","Stefan","Klaus","Wolfgang","Thomas","Peter","Andreas","Christian","Manfred" },

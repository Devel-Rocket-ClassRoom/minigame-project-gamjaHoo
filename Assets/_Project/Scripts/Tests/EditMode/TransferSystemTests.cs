...
namespace FMLite.Tests
{
    public class TransferSystemTests
    {
        ...
        public void T1_MarketValue_Deterministic()
        {
            ...
            var p1 = NewPlayer(1, ca: 150, pa: 170, age: 24, position: Position.ST, contractYears: 4);
            s1.AddPlayer(p1); c1.seniorSquadIds.Add(1);
        ...
        public void T2_MarketValue_SuperstarVsAverage_Over10x()
        {
            ...
            var avg  = NewPlayer(1, ca: 100, pa: 100, age: 25, position: Position.CM, contractYears: 3);
            var star = NewPlayer(2, ca: 180, pa: 200, age: 22, position: Position.ST, contractYears: 5);
            ...
            Assert.Greater(starValue, avgValue * 10,
        ...
        {
            var (state, _, _) = BuildState();
            var young = NewPlayer(1, ca: 130, pa: 150, age: 19, position: Position.CM, contractYears: 3);
            var peak  = NewPlayer(2, ca: 130, pa: 150, age: 25, position: Position.CM, contractYears: 3);
            var old   = NewPlayer(3, ca: 130, pa: 150, age: 31, position: Position.CM, contractYears: 3);
            var done  = NewPlayer(4, ca: 130, pa: 150, age: 36, position: Position.CM, contractYears: 3);
        ...
        {
            var (state, _, _) = BuildState();
            var p1 = NewPlayer(1, ca: 130, pa: 130, age: 25, position: Position.CM, contractYears: 1);
            var p4 = NewPlayer(2, ca: 130, pa: 130, age: 25, position: Position.CM, contractYears: 4);
        ...
        {
            var (state, _, _) = BuildState();
            var st = NewPlayer(1, ca: 130, pa: 130, age: 25, position: Position.ST, contractYears: 3);
            var gk = NewPlayer(2, ca: 130, pa: 130, age: 25, position: Position.GK, contractYears: 3);
            var stInjured = NewPlayer(3, ca: 130, pa: 130, age: 25, position: Position.ST, contractYears: 3);
            ...
            Assert.That(vInjur, Is.LessThan((int)(vSt * 0.6)),
        ...
        {
            Assert.IsTrue (TransferSystem.IsTransferWindowOpen(new DateTime(2026, 6, 1),  _balance), "T6: 6/1 여름 시작");
            Assert.IsTrue (TransferSystem.IsTransferWindowOpen(new DateTime(2026, 8, 31), _balance), "T6: 8/31 여름 끝");
            Assert.IsFalse(TransferSystem.IsTransferWindowOpen(new DateTime(2026, 9, 1),  _balance), "T6: 9/1 닫힘");
            Assert.IsFalse(TransferSystem.IsTransferWindowOpen(new DateTime(2026, 11, 15),_balance), "T6: 11/15 닫힘");
            Assert.IsTrue (TransferSystem.IsTransferWindowOpen(new DateTime(2026, 1, 15), _balance), "T6: 1/15 겨울");
            Assert.IsFalse(TransferSystem.IsTransferWindowOpen(new DateTime(2026, 2, 1),  _balance), "T6: 2/1 닫힘");
        }
        ...
        public void T7_SubmitOffer_Pending()
        {
            ...
            var p = NewPlayer(1, ca: 130, pa: 150, age: 25, position: Position.CM, contractYears: 3);
            ...
            var contract = new Contract { weeklyWage = 50_000, startDate = state.currentDate, endDate = state.currentDate.AddYears(4) };
            var offer = TransferSystem.SubmitOffer(p.id, c1.id, c2.id, 3_000_000, contract, state, _balance);
        ...
        public void T8_AI_AcceptVsReject_BasedOnRatio()
        {
            ...
            var contract = new Contract { weeklyWage = 30_000, startDate = state.currentDate, endDate = state.currentDate.AddYears(4) };
            var highOffer = TransferSystem.SubmitOffer(p.id, c1.id, c2.id, (int)(marketValue * 1.5), contract, state, _balance);
            TransferSystem.ProcessOffers(state, _balance);
            // 여름 활성화 기간 안 (currentDate=7/1) — Accepted → 즉시 Completed 일 수도 / 또는 Accepted 유지
            Assert.That(highOffer.status,
            ...
            // 새 선수 — 다른 player 사용
            var p2 = NewPlayer(2, ca: 100, pa: 100, age: 25, position: Position.CM, contractYears: 3);
            p2.currentClubId = c1.id;
            state.AddPlayer(p2);
            c1.seniorSquadIds.Add(p2.id);
            var lowOffer = TransferSystem.SubmitOffer(p2.id, c1.id, c2.id, (int)(marketValue * 0.5), contract, state, _balance);
            TransferSystem.ProcessOffers(state, _balance);
        ...
        public void T9_AcceptedWaiting_ThenAutoCompleteOnWindowOpen()
        {
            ...
            var contract = new Contract { weeklyWage = 50_000, startDate = state.currentDate, endDate = state.currentDate.AddYears(4) };
            int marketValue = TransferSystem.CalculateMarketValue(p, state, _balance);
            var offer = TransferSystem.SubmitOffer(p.id, c1.id, c2.id, (int)(marketValue * 1.5), contract, state, _balance);
        ...
        public void T10_SearchPlayers_Filter()
        {
            var (state, c1, c2) = BuildState();
            // 다양한 선수 추가
            var st1 = NewPlayer(1, ca: 130, pa: 150, age: 22, position: Position.ST, contractYears: 3); st1.currentClubId = c2.id;
            var st2 = NewPlayer(2, ca: 100, pa: 110, age: 30, position: Position.ST, contractYears: 2); st2.currentClubId = c2.id;
            var cm  = NewPlayer(3, ca: 130, pa: 140, age: 24, position: Position.CM, contractYears: 3); cm.currentClubId  = c2.id;
            var user= NewPlayer(4, ca: 130, pa: 150, age: 22, position: Position.ST, contractYears: 3); user.currentClubId = c1.id;
            state.AddPlayer(st1); state.AddPlayer(st2); state.AddPlayer(cm); state.AddPlayer(user);
            ...
            {
                position = Position.ST, minAge = 20, maxAge = 25,
                minCA = 120, maxCA = 200, excludeUserClub = true,
            };
        ...
        private (GameState state, Club c1, Club c2) BuildState()
        {
            ...
            var c1 = new Club { id = 1, name = "FromClub", reputation = 60, leagueId = 1, finance = new Finance { money = 50_000_000 } };
            var c2 = new Club { id = 2, name = "ToClub",   reputation = 70, leagueId = 1, finance = new Finance { money = 50_000_000 } };
            state.AddClub(c1); state.AddClub(c2);
            return (state, c1, c2);
        }

        private Player NewPlayer(int id, int ca, int pa, int age, Position position, int contractYears)
        {

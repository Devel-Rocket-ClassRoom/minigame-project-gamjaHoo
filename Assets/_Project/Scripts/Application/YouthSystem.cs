...
namespace FMLite.Application
{
    public static class YouthSystem
    {
        ...
        public static YouthIntake GenerateIntake(
            Club club, GameState state, GameBalanceSO balance, LeagueConfigSO leagueConfig)
        {
            ...
            EventBus.Publish(new YouthIntakeAvailableEvent
            {
                intakeId = intake.id,
                clubId   = club.id,
            });
        ...
        public static void UseRerollToken(
            YouthIntake intake, Club club, GameState state,
            GameBalanceSO balance, LeagueConfigSO leagueConfig)
        {
            ...
            var toRemove = intake.candidatePlayerIds
                .Where(id => !intake.signedPlayerIds.Contains(id))
                .ToList();
            ...
            EventBus.Publish(new YouthRerolledEvent
            {
                intakeId         = intake.id,
        ...
        public static void SignPlayers(
            YouthIntake intake, IList<int> playerIds, Club club, GameState state)
        {
            if (intake == null)    throw new ArgumentNullException(nameof(intake));
            ...
            foreach (var id in playerIds)
            {
                ...
                {
                    Debug.LogWarning($"[YouthSystem] SignPlayers: id={id} 는 candidatePlayerIds 외 — 스킵");
                    continue;
                }
                var player = state.GetPlayer(id);
                if (player == null)
                {
                    Debug.LogWarning($"[YouthSystem] SignPlayers: id={id} GameState 에서 not found — 스킵");
                    continue;
                }
            ...
            {
                if (signedSet.Contains(id)) continue;
                state.RemovePlayer(id);
            ...
            {
                intakeId         = intake.id,
                signedPlayerIds  = signedSet.ToList(),
            });
        }
            ...
            GameBalanceSO balance, LeagueConfigSO leagueConfig)
        {
            ...
            var facility = GameDatabase.GetFacilityLevel(FacilityType.Youth, club.facilities.youthLevel);
            if (facility == null)
            {
                Debug.LogWarning($"[YouthSystem] FacilityLevelSO(Youth, lv={club.facilities.youthLevel}) not found — Lv1 폴백");
                facility = GameDatabase.GetFacilityLevel(FacilityType.Youth, 1);
                if (facility == null)
                {
                    Debug.LogError("[YouthSystem] FacilityLevelSO(Youth, lv=1) 도 없음 — intake 빈 풀 반환");
                    return;
                }
            }
            ...
            for (int i = 0; i < poolSize; i++)
            {
                ...
                var player = PlayerGenerator.Generate(
                    rng, club.reputation, position, age, nat,
                    clubId: -1,                            // 미소속
                    youthClubId: club.id,                  // 인스펙션 출처
                    origin: PlayerOrigin.YouthIntake,
                    state.currentDate, balance);
        ...
        {
            if (state.userClubId < 0) return 0;
            var userClub = state.GetClub(state.userClubId);
            if (userClub == null) return 0;
        ...
        private static int SampleYouthPA(Random rng, FacilityLevelSO facility, GameBalanceSO balance)
        {
        ...
        private static int SampleYouthAge(Random rng, GameBalanceSO balance)
        {
            ...
            if (total <= 0) return ages[rng.Next(3)];
            double threshold = rng.NextDouble() * total;
            double cumulative = 0;
            for (int i = 0; i < 3; i++)
            {
                cumulative += weights[i];
                if (cumulative >= threshold) return ages[i];
            }
            return ages[2];
        }

        private static string SampleYouthNationality(Random rng, string leagueCountry, GameBalanceSO balance)
        {
            ...
            if (others.Count == 0) return leagueCountry;

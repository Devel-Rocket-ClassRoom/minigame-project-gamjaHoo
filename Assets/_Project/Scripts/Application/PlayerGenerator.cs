    ...
    public static class PlayerGenerator
    {
            ...
            GameBalanceSO balance)
        {
            ...
            if (age < balance.minAge || age > balance.maxAge)
                Debug.LogWarning($"[PlayerGenerator] age {age} out of range [{balance.minAge}, {balance.maxAge}].");
            ...
            var info     = BuildPersonalInfo(rng, targetPosition, age, nationalityCode, currentDate, balance);
        ...
        private static void FillOutfieldProfile(Random rng, Stats s, double mean, PositionSO posSO, GameBalanceSO b)
        {
            double techMean = mean + (posSO != null && posSO.emphasizesTechnical ? b.statEmphasisBonus : -b.statEmphasisPenalty);
            double mentMean = mean + (posSO != null && posSO.emphasizesMental    ? b.statEmphasisBonus : -b.statEmphasisPenalty);
            double physMean = mean + (posSO != null && posSO.emphasizesPhysical  ? b.statEmphasisBonus : -b.statEmphasisPenalty);
            ...
            s.gk.ApplyToAll(_ => ClampStat(rng.NextNormal(b.outfieldGkStatBase, b.statStdDev * 0.5)));
        }
        ...
        private static List<int> SelectTraits(Random rng, GameBalanceSO b)
        {
            ...
            if (rng.NextDouble() < b.traitProbabilityPerPlayer)
            {
                ...
                while (rng.NextDouble() < b.additionalTraitProbability)
                    if (!TryAddTrait(rng, selected, usedGroups)) break;
            }
        ...
        {
            var pool = GameDatabase.AllTraits
            ...
            if (chosen.exclusionGroupId != 0) usedGroups.Add(chosen.exclusionGroupId);
            return true;
        }
        ...
        private static PersonalInfo BuildPersonalInfo(
            Random rng, Position pos, int age, string nationalityCode,
            DateTime currentDate, GameBalanceSO b)
        {
            var    namePool  = GetNamePoolByCode(nationalityCode);
            string firstName = namePool != null && namePool.firstNames.Count > 0
                ? namePool.firstNames[rng.Next(namePool.firstNames.Count)]
                : "Unknown";
            string lastName = namePool != null && namePool.lastNames.Count > 0
        ...
        private static Foot SampleFoot(Random rng, GameBalanceSO b)
        {
            ...
            return rng.WeightedSample(feet, f => f switch
            {
        ...
        private static List<Position> GenerateSecondaryPositions(Random rng, Position primaryPos, GameBalanceSO b)
        {
            var primarySO = GetPositionSO(primaryPos);
            if (primarySO != null && primarySO.isGoalkeeper) return new List<Position>();
        ...
        private static Position PickSecondaryPosition(Random rng, PositionSO primarySO, HashSet<Position> exclude)
        {
            var pool = GameDatabase.AllPositions
                .Where(p => !exclude.Contains(p.position) && !p.isGoalkeeper)
                .ToList();

            if (pool.Count == 0) return primarySO?.position ?? Position.CM;

            return rng.WeightedSample(pool, p =>
            {
                if (primarySO == null) return 0.05;
                var aff = primarySO.affinities.FirstOrDefault(a => a.position == p.position);
                return aff != null ? (double)aff.weight : (double)primarySO.fallbackAffinityWeight;
            }).position;
        }

        // ── 6단계: 계약 · 상태 ────────────────────────────────────────

        private static Contract BuildContract(Random rng, int ca, DateTime currentDate, GameBalanceSO b)
        {
            return new Contract
            {
        ...
        private static PlayerState BuildState() => new PlayerState
        {
            fatigue           = 0,
            morale            = 50,
            form              = 50,
        ...
        private static NamePoolSO GetNamePoolByCode(string code)
        {
            ...
            if (country == null)
            {
                Debug.LogWarning($"[PlayerGenerator] Country '{code}' not found, falling back to ENG.");
                country = GameDatabase.AllCountries.FirstOrDefault(c => c.code == "ENG");
                if (country == null) return null;
            }
            var pool = GameDatabase.GetNamePool(country.id);
            if (pool == null)
                Debug.LogWarning($"[PlayerGenerator] No NamePool for country '{code}' (id {country.id}).");
            return pool;
        }

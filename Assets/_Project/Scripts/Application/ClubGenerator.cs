...
namespace FMLite.Application
{
    public static class ClubGenerator
    {
        ...
        private static int[] AllocateTierCounts(float[] ratios, int n)
        {
            ...
            if (remainder > 0)
            {
                var order = Enumerable.Range(0, tierCount)
                    .OrderByDescending(i => raw[i] - counts[i])
                    .ToArray();
                for (int k = 0; k < remainder; k++) counts[order[k]] += 1;
            }
            else if (remainder < 0)
            {
        ...
        private static Club BuildClub(
            Random rng, int idx, int rep,
            LeagueConfigSO cfg, GameBalanceSO b,
            DateTime currentDate, int leagueId, int startClubId)
        {
            ...
            int    money      = Math.Max(b.financeFloor,
        ...
        private static int SampleFacilityLevel(Random rng, double mu, GameBalanceSO b) =>
            Math.Clamp(
                ...
                b.minFacilityLevel, b.maxFacilityLevel);
        ...
        private static Player GeneratePlayer(
            Random rng, Club club, Position pos,
            string leagueCountry, DateTime currentDate, GameBalanceSO b)
        {
        ...
        private static int SampleAge(Random rng, GameBalanceSO b)
        {
            ...
            int bucket = rng.WeightedSample(buckets, i => i switch
            {
        ...
        {
            if (rng.NextDouble() < b.primaryNationalityRatio) return leagueCountry;

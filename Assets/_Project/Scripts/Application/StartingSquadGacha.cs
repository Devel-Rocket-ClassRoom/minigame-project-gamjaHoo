...
namespace FMLite.Application
{
    public static class StartingSquadGacha
    {
        ...
        private static readonly HashSet<Position> GkPositions = new HashSet<Position>
        {
            Position.GK,
        };
        private static readonly HashSet<Position> DfPositions = new HashSet<Position>
        {
            Position.CB,
            Position.LB,
            Position.RB,
        ...
        private static readonly HashSet<Position> MfPositions = new HashSet<Position>
        {
            Position.DM,
            Position.CM,
            Position.AM,
        ...
        private static readonly HashSet<Position> AtPositions = new HashSet<Position>
        {
            Position.LW,
            Position.RW,
            Position.ST,
        ...
        {
            if (GkPositions.Contains(pos))
                return Line.GK;
            if (DfPositions.Contains(pos))
                return Line.DF;
            if (MfPositions.Contains(pos))
                return Line.MF;
            if (AtPositions.Contains(pos))
                return Line.AT;
        ...
        public static SquadEvaluation EvaluateSquad(
            Club club,
            GameState state,
        ...
        )
        {
            ...
            // 3단계: 명성 대비 비율 → 5단계 티어
            double expectedMean = balance.caRepBase + (double)balance.caRepCoeff * club.reputation;
            if (expectedMean < 1.0)
                expectedMean = 1.0; // 0 나누기 방어
        ...
        private static TierGrade GradeLine(
            List<int> abilities,
            double expectedMean,
            GameBalanceSO b
        )
        {
            ...
            if (ratio >= b.tierEliteRatio - TierEpsilon)
                return TierGrade.Elite;
            if (ratio >= b.tierStrongRatio - TierEpsilon)
                return TierGrade.Strong;
            if (ratio >= b.tierAverageRatio - TierEpsilon)
                return TierGrade.Average;
            if (ratio >= b.tierWeakRatio - TierEpsilon)
                return TierGrade.Weak;
            return TierGrade.Poor;
        }

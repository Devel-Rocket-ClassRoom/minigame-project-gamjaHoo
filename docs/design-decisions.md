...
- **Tier 1 (이벤트 존재, 라우팅만)**: 부상발생(`PlayerInjuredEvent` → Match), **부상 복귀(`PlayerInjuryRecoveredEvent` → Match)**, 월간/시즌 시상(SeasonAwardSystem → Award 카테고리 — #66 V1.x 노트의 Award 라우팅을 V1.0 로 당김), 유스 성장(`PlayerStatChangedEvent` → Youth).
- **Tier 2 핵심 (신규 이벤트 4종)**: 선수 불만(`PlayerUnhappyEvent`/Morale), 피로 누적(`PlayerFatiguedEvent`/Morale), 순위 변동(`StandingsChangedEvent`/League), 계약 만료 임박(`ContractExpiringEvent`/Transfer).
- 정책: 전부 `deadline=null`, 대부분 `priority=Low`, Q1(기한만료 무효)·Q2(시즌종료 시 읽은 것 삭제) 유지. 카테고리 탭+우선순위 정렬(Stage B)로 폭주해도 중요 알림 안 묻힘.

> **구현 정정 (2026-06-07, R.5):** 초안은 부상 복귀를 신규 `PlayerRecoveredEvent` 로 적었으나, **이미 V0.5 D.4 `PlayerInjuryRecoveredEvent` 가 존재** → Tier1 라우팅으로 흡수 (event-bus-catalog / v1.0-tasks 와 일치). 따라서 Tier2 신규 = **4종**. 전부 발화 지점에서 **유저 구단 한정 게이팅** (피로/불만/계약만료/순위변동), Tier1 은 InboxRouter 에서 유저 구단(유스성장은 유저 유스) 게이팅. **추가 연결**(spec 외, 사용자 latitude): `TransferCompletedEvent`(영입/방출) / `LoanReturnedEvent`(임대 복귀) 라우팅 + 월간 선수상 `AwardWonEvent` 발행.
...
| 2026-06-07 | #76 구현 (R.5) | 인박스 확장 구현. `InboxCategory.League` append / Tier1 라우팅(PlayerInjured·PlayerInjuryRecovered·PlayerStatChanged-유스·AwardWon-시즌+월간, InboxRouter 유저구단 게이팅) / Tier2 신규 4종(PlayerUnhappy=OnPromiseBroken 하향교차 / PlayerFatigued=MatchPostProcessor 상향교차 / StandingsChanged=BackgroundSimulator 배치·**MatchPostProcessor 에서 정정** / ContractExpiring=DailyProcessor Day==1 ==임계, 발화 유저구단 게이팅) / **추가 연결**(TransferCompleted·LoanReturned 라우팅, 월간 AwardWon 발행). 초안 "부상복귀=신규 PlayerRecoveredEvent" → 기존 PlayerInjuryRecoveredEvent Tier1 로 정정. `GameBalanceSO.fatigueAlertThreshold(50)`/`contractExpiryAlertMonths(6)` 신규. 로컬라이즈 키 inbox_*_fmt 11 + league 카테고리 + award_type_* 9 보완 → **에셋 재생성 chore 필요**. `InboxTests` T17~T28 / `LocalizationV10Tests` T10. |
